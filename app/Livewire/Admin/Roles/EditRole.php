...
class EditRole extends Component
{
    ...
    {
        return [
            'name' => ['required', 'min:3', Rule::unique('roles')->where('tenant_id', tenant('id'))->ignore($this->role->id)],
            'icon' => 'required'
        ];
    }
    ...
    public function mount(Role $role)
    {
        ...
        $this->role = $role;
        $this->name = $role->name;
        $this->icon = $role->icon ?? 'eye'; // Als geen icoon, gebruik 'eye'
        $this->selectedIcon = [ // Selecteer het juiste pad + naam
            'name' => $role->icon ?? 'eye',
            'path' => $this->getIconPath($role->icon ?? 'eye')
        ];
        // Modules + permissions ophalen voor user_type = 1 = tenant-superadmin
        $this->modules = Module::where('user_type', 1)
            ->with('permissions') // eager load permissions
            ->get()
            ->map(function ($module) {
                return [
                    'module' => $module->name, // Naam van de module
                    'permissions' => $module->permissions->map(function ($perm) {
                        return ['id' => $perm->id, 'name' => $perm->name]; // ID + naam van elke permission
                    })->toArray(),
                ];
            })->toArray();
        // Permissions die deze rol al heeft, inlezen
        $this->selectedPermissions = $role->permissions->pluck('id')->toArray();
    }

    public function toggleIconPicker()
    {
        $this->showIconPicker = !$this->showIconPicker; // Toggle de icon picker
    }

    public function selectIcon($name, $path)
    {
        $this->icon = $name; // stel gekozen icoon-naam
        $this->selectedIcon = [ // stel gekozen icoon in
            'name' => $name,
            'path' => $path
        ];
        $this->showIconPicker = false; // Verberg picker
    }
    ...
    {
        $icons = $this->availableIcons; // Verkrijg alle beschikbare iconen
        return $icons[$iconName]['path'] ?? $icons['eye']['path']; // Geef pad of fallback
    }

    public function getAvailableIconsProperty()
    {
        try {
            // Laad iconen uit node_modules als JSON
            $icons = json_decode(file_get_contents(base_path('node_modules/@heroicons/react/24/outline/index.js')), true);
        } catch (\Exception $e) {
            // Fallback iconen als de echte niet geladen kunnen worden
            return [
    ...
    public function save()
    {
        ...
        $validated = $this->validate(); // Validatie

        DB::transaction(function () use ($validated) {
            $this->role->update($validated); // Update rol met nieuwe naam/icon

            // Permissions synchroniseren (alleen die van modules met user_type 1)
            if (!empty($this->selectedPermissions)) {
                $permissions = Permission::whereIn('id', $this->selectedPermissions)
                    ->whereHas('module', function ($query) { // Filter permissions by user type
                        // Alleen permissions van modules met user_type 1
                        $query->where('user_type', 1); // extra beveiliging
                    })
                    ->get();
                $this->role->syncPermissions($permissions); // Koppel opnieuw
            } else {
                $this->role->syncPermissions([]); // Verwijder alle gekoppelde permissions
            }
        });

        session()->flash('message', __('Rol succesvol bijgewerkt.'));
        session()->flash('message_type', 'success');

        return $this->redirect(route('admin.roles.index'), true);
    }
